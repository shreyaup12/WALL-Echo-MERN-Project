...
const RoomManager = ({ user, onRoomSelect, onClose, currentRoom }) => {
  ...
  const loadUserRooms = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4002/api/v1/room/', {
        headers: {
  ...
  const createRoom = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch('http://localhost:4002/api/v1/room/create', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const joinRoom = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:4002/api/v1/room/join/${joinRoomId.trim()}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const leaveRoom = async (roomId) => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:4002/api/v1/room/${roomId}/leave`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
